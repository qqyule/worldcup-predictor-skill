...
一个面向 Codex、Claude Code 等 Agent 环境的世界杯预测 Skill。它使用离线审计快照和内置确定性核心，完成单场 90 分钟概率预测、2026 世界杯赛事模拟，以及中国足球彩票 3/1/0 娱乐参考清单整理。

本仓库不依赖 Next.js、数据库、在线抓取或大模型计算概率。LLM 只能解释结果，不能替代规则与概率计算。
...
```text
使用 worldcup-predictor，根据审计快照预测法国对巴西的 90 分钟胜平负概率。
使用 worldcup-predictor，按固定 seed 模拟 10,000 次 2026 世界杯并解释冠军概率。
使用 worldcup-predictor，根据 14 场 issue JSON 生成均衡型 3/1/0 娱乐参考清单。
```
...
本仓库的样例数据仅用于功能演示和测试，不代表官方赛程、真实球队实力或实际预测结论。
